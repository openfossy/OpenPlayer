...
import com.openfossy.openplayer.data.media.MediaStoreHelper
import com.openfossy.openplayer.data.repository.VideoRepository
import com.openfossy.openplayer.player.engine.MPVPlayerEngine
import com.openfossy.openplayer.ui.navigation.AppNavigation
import com.openfossy.openplayer.ui.theme.NosvedPlayerTheme
...
class MainActivity : ComponentActivity() {
    ...
    private val pipReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            when (intent.action) {
                "com.openfossy.openplayer.PIP_PREV" -> playerViewModel.playPrevious()
                "com.openfossy.openplayer.PIP_PLAY_PAUSE" -> {
                    playerViewModel.togglePlayback()
                    updatePipActions()
                }
                "com.openfossy.openplayer.PIP_NEXT" -> playerViewModel.playNext()
            }
        }
    }
    ...
    private fun buildPipActionsList(): List<android.app.RemoteAction> {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            // 1. Prev Action
            val prevIntent = Intent("com.openfossy.openplayer.PIP_PREV")
            ...
            val playPauseIntent = Intent("com.openfossy.openplayer.PIP_PLAY_PAUSE")
            ...
            // 3. Next Action
            val nextIntent = Intent("com.openfossy.openplayer.PIP_NEXT")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = IntentFilter().apply {
            addAction("com.openfossy.openplayer.PIP_PREV")
            addAction("com.openfossy.openplayer.PIP_PLAY_PAUSE")
            addAction("com.openfossy.openplayer.PIP_NEXT")
        }
    ...
    private fun logExternalVideoHistory(uri: Uri) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                val metadataResult = com.openfossy.openplayer.data.mediainfo.MediaInfoOps.extractBasicMetadata(
                    this@MainActivity,
                    uri,
                    fileName
                )
                ...
                val video = com.openfossy.openplayer.domain.model.Video(
                    uri = uri.toString(),
                    title = fileName,
                    duration = duration,
                    folderName = "",
