...
import com.openfossy.openplayer.util.getAvailableStorageVolumes
...
import com.openfossy.openplayer.ui.screens.videolist.state.PathSegment
...
class VideoListViewModel(
                    ...
                    val videos = items.map { item ->
                        ...
                        if (item.size > 0 && item.duration > 0) {
                            ...
                            if (cached != null) {
                                ...
                                val extracted = com.openfossy.openplayer.util.getVideoMetadata(repository.context, item.uri)
                                ...
                                metadataDao.insertOrUpdate(
                                    com.openfossy.openplayer.data.database.CachedVideoMetadata(
                                        uri = uriStr,
                                        size = finalSize,
    ...
    fun selectFolder(folder: VideoFolder?) {
        ...
        if (folder != null) {
            ...
            com.openfossy.openplayer.domain.thumbnail.ThumbnailRepository.getInstance(repository.context)
    ...
    fun updateNewVideosDaysThreshold(days: Int) {
        viewModelScope.launch {
            viewSettingsRepo.updateNewVideosDaysThreshold(days)
        }
    }
}
